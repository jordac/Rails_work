...
Then /the director of "(.*)" should be "(.*)"/ do |movie, dir|
  # makes sure correct director for movie object
  movie_obj = Movie.find_by_title(movie)
  director = movie_obj[:director]
  if director != dir
    flunk "#{dir} != #{Movie.find_by_title(movie).director}"
  end
end

Given /I am on the details page for "(.*)"/ do |mt|
  movie_id = Movie.find_by_title(mt)[:id]
  #puts movie_id
